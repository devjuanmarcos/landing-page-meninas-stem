...
import { getNewsById } from "@/app/actions/newsActions";

export const NoticiaIndividual = () => {
  const [isMounted, setIsMounted] = React.useState<boolean>(false);
  const [noticia, setNoticia] = React.useState<NewsType | undefined>(undefined);
  ...
  React.useEffect(() => {
    ...
    async function fetch() {
      try {
        const res = await getNewsById(params.newsId as string);
        setNoticia(res.news);
      } catch (error) {
        console.log(error);
      } finally {
      ...
      <React.Fragment>
        ...
        >
          <div className="py-7 container w-full mx-auto lg:px-24 px-10">
            <span className="title-card text-white">{noticia.categoryLabel}</span>
          </div>
        </div>
